...
//Alaa s
	public void autoResolve(Army attacker, Army defender) throws FriendlyFireException {
		...
		while (attacker.getUnits().size() != 0 && defender.getUnits().size() != 0) {
			...
			if (turn == 1) {
				int tmp = unit2.getCurrentSoldierCount();
				unit1.attack(unit2);
				s += autoResolveLog("Defender City Army", tmp, unit2.getCurrentSoldierCount());
			} else {
				int tmp = unit1.getCurrentSoldierCount();
				unit2.attack(unit1);
				s += autoResolveLog("Your Army", tmp, unit1.getCurrentSoldierCount());
			}
			turn = turn == 1 ? 0 : 1;
		...
		if (attacker.getUnits().size() != 0) {
			...
			s+= "You won the battle and occupied the city!";
		}
		else
			s+= "You lost the battle";
		listener.onAutoResolveLog(s);
	}

	public String autoResolveLog(String whichArmy, int from, int to) {
		return whichArmy + " was attacked and lost " + (from - to) + " soldiers. \n";
	}
	// Alaa e
